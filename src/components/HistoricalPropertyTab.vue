  ...
  created() {
    getTableInfo(
      ...
      if ("results" in data) {
        ...
        sortedData = sortedData.filter(a => a.document_type === "DEED" || a.document_type === "DEED SHERIFF" || a.document_type === "DEED OF CONDEMNATION" || a.document_type === " DEED LAND BANK")
        for (let i in sortedData) {
          let row = sortedData[i];
          row.name = row.grantors;
          if(i == 0){
            row.start = new Date(row.year_built !== "0000" ? row.year_built : row.receipt_date);
          }
          else{
