  ...
  <div class="property">
    <div v-if="result">
      <sui-container text>
        ...
        <a :href="result.link_cyclomedia_street_view" target="_blank"
          >Link to Cyclomedia Street View</a
        >,
        <a :href="result.link_property_phila_gov" target="_blank"
          >Link to property.phila.gov</a
        >,
        <a :href="result.link_license_inspections" target="_blank"
...
export default {
    ...
    buildingDescription() {
      ...
      if (this.result !== null) {
        return (
          ...
          this.result.year_built +
          year_built_estimate_str +
          ", and is described as a " +
          this.result.category_code_description +
          ", " +
  ...
  created() {
    getTableInfo("properties", "parcel_number", this.parcelNumber).then(
      data => {
