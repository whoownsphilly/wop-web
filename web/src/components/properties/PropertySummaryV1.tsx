...
const PropertySummary: Component = (props) => {
    const { id }  = props
    ...
    createEffect(async () => {
        const result= await getPropertyBasicsPageInfo(id, violationDate())
        setProperty(Property.toProperty(result))
        const result2 = await getPropertyLatestOwnerDetailsInfo(id, violationDate())
